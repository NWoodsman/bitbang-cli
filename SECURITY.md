...
[**Report a vulnerability**](https://github.com/richlegrand/bitbang-cli/security/advisories/new).
...
BitBang's security model is that the connection is end-to-end encrypted between the browser
or client and the device, and the signaling server routes without being able to read the
traffic or insert itself into it. Anything that breaks that is in scope.
...
- Reaching a network target the operator did not authorize, including through the HTTP
  proxy
- Remote code execution on the device, or in the connecting browser
- Resource exhaustion triggered by a peer, where a bounded input causes unbounded
  allocation or work
- Weaknesses in the release pipeline that could result in a tampered binary
...
- Anyone holding a valid access link having the access that link grants. Links are
  capabilities: whoever holds one can connect, which is the intended behavior. Ways to
  obtain a link you were never given are in scope.
- Attacks that require an already-compromised device, or an operating system account on
  the machine running the listener
- Volumetric denial of service, meaning simply sending more traffic than a host can handle.
  Bugs where a small input causes disproportionate resource use are in scope, as above.
